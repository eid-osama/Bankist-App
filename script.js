...
function formatMov(value, locale, currency) {
...
function displayMovements(acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formatedMov = formatMov(mov, acc.locale, acc.currency);

    const diplayDate = formatDate(date, acc.locale);
...
function starLogOutTimer() {
  ...
  function tick() {
    ...
    // wehn timer reach zero log out user
    if (time === 0) {
      clearInterval(time);
